...
- schema: `components/quote/quote.schema.json`
- template: `components/quote/quote.html`
- css: `components/quote/quote.css`
- documentation: `components/quote/quote.md`
- examples: `components/quote/examples.json`

Load `dist/sitekit.css`; root selector `.sk-quote`. No component JavaScript export.
...
Declared variants: pull, compact.
...
| --- | --- | --- |
| default | source template | components/quote/quote.html |
| focus | CSS selector; may target descendants | components/quote/quote.css |
...
```css
.sk-quote figcaption { color: var(--sk-text-muted); font-family: var(--sk-font-mono); font-size: var(--sk-type-size-xs); text-transform: uppercase; }
```
...
```html
<figure class="sk-quote"><blockquote>SiteKit keeps interface patterns inspectable and reusable.</blockquote><figcaption>Product team</figcaption></figure>
```
...
Actual CSS variables: `--sk-border-2`, `--sk-border-default`, `--sk-focus-offset`, `--sk-focus-ring`, `--sk-focus-width`, `--sk-font-mono`, `--sk-space-3`, `--sk-space-5`, `--sk-surface-card`, `--sk-text-muted`, `--sk-type-size-lg`, `--sk-type-size-xs`.

## Caveats and documentation

- Use real approved quote content; source is demonstration copy, no special compact treatment.
