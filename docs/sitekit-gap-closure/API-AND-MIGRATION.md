...
- Icon: size is data-size=sm/md/lg; boxed is data-icon-variant=boxed. Meaningful wrappers need role=img and aria-label; decorative wrappers need aria-hidden=true. Inline SVG and local sprite use are markup compositions. CSS cannot assign ARIA. The bundled Tabler v3.46.0 sprite uses its included MIT license; sk-glyph provides compact inline sizing. Consumer-supplied icons retain their own licenses.
